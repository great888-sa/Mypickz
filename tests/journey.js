...
(async () => {
  ...
  await must('ش٢٢ · أ-١٢-١: الوجهة الافتراضية بقاعدتها · وسم «مدينتي» حتى مدينتين والأخيرة استعمالًا · إعلان التحول مرة · مثال الملاحظة', async () => {
    ...
    // r70n2: المدينة المحفوظة بلا محتوى لا تُفرض عند الفتح؛ وتُعتمد إن اختارها المستخدم بهذه الجلسة — على دالة الاختيار وحدها بلا آثار
    const sv = B.x("(function(){ var k = { cpc: userListData.cityPlaceCounts, lc: userListData.listCity, cc: userListData.customCities, mc: userListData.myCities, sc: plSessionCity }; userListData.customCities = (userListData.customCities || []).concat([{ id: 'mylist_gamma', name: 'Gamma', country: 'Testland' }]); userListData.cityPlaceCounts = {}; userListData.myCities = []; userListData.listCity = 'mylist_gamma'; plSessionCity = null; var a = plInitialCityPick(); plSessionCity = 'mylist_gamma'; var b = plInitialCityPick(); userListData.cityPlaceCounts = { mylist_gamma: 2 }; plSessionCity = null; var c = plInitialCityPick(); userListData.cityPlaceCounts = k.cpc; userListData.listCity = k.lc; userListData.customCities = k.cc; userListData.myCities = k.mc; plSessionCity = k.sc; return JSON.stringify({ a: a ? a.id : null, b: b ? b.id : null, c: c ? c.id : null }); })()");
    ok('ش٢٢ · r70n2: المحفوظة بلا محتوى لا تُفرض (null) · تُعتمد إن اختيرت بالجلسة · وتُعتمد حين يصير لها محتوى', sv === '{"a":null,"b":"mylist_gamma","c":"mylist_gamma"}', 'got ' + sv);
    const ncEdit = B.x("(function(){ var k = { c: myListCityId, d: myCityListData, k2: myListCountry, cc: userListData.customCities }; userListData.customCities = (userListData.customCities || []).concat([{ id: 'mylist_delta', name: 'Delta', country: 'Testland' }]); myListCityId = null; myCityListData = null; myListCountry = 'Testland'; plPanel = 'cities'; __pk.plCities = __pk.plCities || {}; __pk.plCities.edit = false; renderPlacesMine(); var h = document.getElementById('plBody').innerHTML; plPanel = null; myListCityId = k.c; myCityListData = k.d; myListCountry = k.k2; userListData.customCities = k.cc; return h.includes('pickerToggleEdit(') && h.includes('class=\"homebtn') && !h.includes('class=\"del\"') && h.includes('Delta'); })()");
    ok('ش٢٢ · r70o3: بفرع «بلا مدينة» لوحة المدن تعرض 🏠 دائمًا بلا وضع تحرير، وزر Edit للحذف وحده (قرار المالك)', ncEdit === true, '');
    ...
    // r70p2: الوسم يعود بعد إعادة التحميل من المستند (كان يضيع بكل دخول)
    const reload = await B.x("(async function(){ var k = { mc: userListData.myCities, ha: userListData.homeAnnounced }; await mpData.userLists.merge(currentUser.uid, { myCities: ['mylist_alpha'], homeAnnounced: true }); await loadUserList(); var out = JSON.stringify({ mc: userListData.myCities, ha: userListData.homeAnnounced }); userListData.myCities = k.mc; userListData.homeAnnounced = k.ha; await mpData.userLists.merge(currentUser.uid, { myCities: k.mc, homeAnnounced: k.ha }); return out; })()");
    ok('ش٢٢ · r70p2: وسم مدينتي وعلم الإعلان يُقرآن عند التحميل فلا يضيعان بالخروج والدخول', reload === '{"mc":["mylist_alpha"],"ha":true}', 'got ' + reload);
