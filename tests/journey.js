...
(async () => {
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    ok('١٦هـ · ر٧٢-أ-٢ب: الإفصاح بنافذة التطبيق (لا confirm المتصفح) · بطاقة الرحلة بطبقة الشخص = بطاقة السوق · فشل المتابعة يُسجَّل بلوحة التوقيت', tplCount('function openConfirmModal(title, hint){', 1).ok && tplCount("confirm('Following is visible", 0).ok && tplCount("saveMsg: 'Copy this trip into yours — stage 3', exportHandler: \"exportOtherTrip('\" + attrStr(t.id) + \"')\", openHandler: \"openCommunityTrip('\" + attrStr(t.id) + \"')\"", 1).ok && tplCount("logTiming('[CUR] follow failed", 1).ok && tplCount("showToast('Could not follow · ' + ((e && e.code)", 1).ok, '');
