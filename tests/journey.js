...
(async () => {
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    const cityDet = B.x("JSON.stringify([cityIdFromAddress('Shop 2104 Building, 1435 Rd 4626, Manama, Bahrain'), cityIdFromAddress('7351, Al Imam Saud Ibn Faysal Rd, As Sahafah, Riyadh 13315'), cityIdFromAddress('8 Rue Tiquetonne, 75002 Paris'), isCityOrCountry('Manama'), isCityOrCountry('Bahrain'), isCityOrCountry('As Sahafah')])");
    ok('١٦هـ · ز-١-ب: مدينة المكان تُستنتج من العنوان (manama · riyadh · paris) · المدن والدول لا تدخل Area', cityDet === '["manama","riyadh","paris",true,true,false]', cityDet);
    B.x("plResolvedCity = 'manama'; myListCityId = 'paris'; plEdit = null; plModalCat = '" + CAT1 + "'; document.getElementById('plName').value = 'Caribou Coffee'; document.getElementById('plUrl').value = 'https://maps.app.goo.gl/x9'; plGeoConfirmed = false; plGeoResult = null; window.__mpGeoStub = { candidates: [] }"); await B.x('savePlPlace()'); await new Promise(r => setTimeout(r, 40)); const mm = String((documentStub.getElementById('geoMismatch') || { innerHTML: '' }).innerHTML || '');
    ok('١٦هـ · ز-١-ب: مدينة المكان تخالف مدينة القائمة → رسالة «seems to be in Manama — adding it to Paris» وزر Switch to Manama بخطوة الخريطة', mm.includes('seems to be in <b>Manama</b>') && mm.includes('Switch to Manama') && mm.includes('geoSwitchCity()'), mm.slice(0, 200));
    await B.x('geoSkip()'); await new Promise(r => setTimeout(r, 40)); B.x("plResolvedCity = ''");
