...
(async () => {
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    ok('١٦هـ · r72n: resetSessionState يصفّر الرحلات والمنتقين والمجتمع والاسم ويرسم «Loading…» · ويُستدعى عند تبدّل المصادقة', reset === '{"t":0,"c":null,"u":0,"v":null,"n":null,"body":true}' && tplCount("if (!user || !currentUser || user.uid !== currentUser.uid) resetSessionState();", 1).ok && tplCount("delete curData[uid]; /* r72n", 1).ok, reset);
