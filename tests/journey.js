...
(async () => {
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    ok('١٦هـ · الشبكة: الموثَّقون فقط (لا Cara) · الأحدث تحديثًا أولًا (Amal 2d قبل Badr 9d) · دوائر بالحرفين · البيانات من قوائمه العامة (Amal: 1 city · 1 trip · Badr بلا محتوى عام) · متابعون بإذنه · updated · «You» على حسابي · صف الرأس', iB > -1 && iA > -1 && iA < iB && !g.includes('Cara') && g.includes('class="curring">AM<') && g.includes('class="curpill">') && g.includes('1 city · 1 trip') && g.includes('— no public content yet') && g.includes('12 followers') && !g.includes('3 followers') && g.includes('updated 2d ago') && g.includes('class="curyou">You<') && g.includes('placeholder="Type a name…"') && g.includes('pick a curator'), 'iB=' + iB + ' iA=' + iA + ' metas=' + (g.match(/class="curmeta">[^<]*/g) || []).join(' | ').slice(0, 300));
    ...
    ok('١٦هـ · المدينة المختارة تُعرض بالمكان نفسه تحت الشرائح (الرأس والشرائح باقيان · الشريحة تحمل اسمها Paris) · للزائر بلا رأي: لا بطاقة رأي أصلًا (r72p — قرار المالك) · مبدّل Top places · 0 / All places · 3 / Trips · 1 · لا Top places بعد', cty.includes('class="curidn"') && cty.includes('Cities · 1<span class="s">Paris ') && !cty.includes('No notes yet') && !cty.includes('Best time of year') && !cty.includes('class="curfacts"') && cty.includes('Paris Top places · 0') && cty.includes('Paris places · 3') && cty.includes('Paris trips · 1') && cty.includes('← Curators') && cty.includes('No top places in Paris yet'), cty.slice(0, 200));
    ...
    ok('١٦هـ · r72p/q1: Top places بعدده الحقيقي (1) · زر ⚑ أحمر برأس صفحة الغير', pk.includes('<b>1</b> Top places') && !pk.includes('picks') && pk.includes('class="curic flag" onclick="openReport(\'profile\', \'cur_a\')"'), pk.slice(0, 160));
    ...
    ok('١٦هـ · r72q-1: الاشتقاق — الاسم المنتهي بـs («Louis’») بالشرائح · اسم المدينة بشرائح شاشتها (Paris …) · الشبكة كبسولة بالحرفين والاسم', pz.includes('<b>Louis’</b>latest updated places') && pz.includes('<b>All Louis’</b>trips') && pz2.includes('Paris Top places · 0') && pz2.includes('Paris places · 1') && tplCount('<span class="curpill"><span class="curring">', 1).ok, pz.slice(0, 160));
