...
(async () => {
  ...
  await must('ش٢٤ · أ-١٢-٣ بطاقة المصادر الستة باليوم الفارغ: أعداد مدينة الرحلة · كل زر ينقل بوضع الإضافة · تزول بأول مكان · لا تظهر للقراءة', async () => {
    ...
    B.x("activeTripId = 'trip_src_x'; tripSrcOpen = false; openActiveTrip()");
    const back = B.x("JSON.stringify({ tab: currentTab, id: currentTripId, open: tripSrcOpen })");
    ok('ش٢٤ · r70o3: «← Trip» يعود إلى الرحلة النشطة والبطاقة مفتوحة · أزرار الشريط بسطر مستقل بثلاثة أثلاث', back === '{"tab":"Trips","id":"trip_src_x","open":true}' && tplCount('onclick="openActiveTrip()">← Trip</button>', 1).ok && tplCount('.atb-head .acts{display:flex; gap:6px; flex:1 1 100%;}', 1).ok, 'got ' + back);
