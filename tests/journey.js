...
(async () => {
  ...
  await must('ع٣ · مكانان برابط متطابق لا يتشاركان المفكرة (المفتاح هوية العنصر — ر٦٨)', async () => {
    ...
    const onCount = (rendered.match(/pc-a bmk on/g) || []).length; // r72q-2: البطاقة الموحَّدة
  ...
  await must('ش١٧ · ٦/و العناوين و٦/ب٢ إنشاء الرحلة على اللوحة المشتركة · المدينة الجديدة تُخلَّد فورًا بعدّاد صفر — ر٧٠د', async () => {
    ...
    B.x("__gaz['FR'] = Promise.resolve([{ id: 999001, n: 'Lyon Test', a: ['Lyon T'], lat: 45.76, lng: 4.83, p: 500000 }])"); // ز-١-ج-٢: المعجم المحاكى (المعرّف والمركز)
  ...
  await must('ش١٨ · ٦/د المجتمع: الجذر والمصدر على الصيغة الموحَّدة (شريحة + حقل) بعدّاد المحتوى لكل مدينة · Go فعلًا رئيسًا — ر٧٠هـ', async () => {
    ...
    B.x("communityTab = 'places'; communityScreenState.places.city = ''; renderCommunityModal(); cmFillCityPick([{ cityId: 'paris', cityName: 'Paris' }, { cityId: 'paris', cityName: 'Paris' }, { cityId: 'rome', cityName: 'Rome' }])");
    const one = screen('communityBody');
    ok('ش١٨ · r72s: الشاشة الواحدة — لا مدخل ولا زر عودة · مبدّل المصدر أولًا · شريحة المدينة بصف واحد · All users + Search by username · My bookmarked + Sort by', B.x('communityScreen') === 'source' && !one.includes('backchip') && !one.includes('cmRootCity') && one.indexOf('>Places<') < one.indexOf('id="cmCityRow"') && one.includes('class="headrow pkrow"') && one.includes('>Search by username<') && one.includes('All users places lists <span class="srcsub">· '), one.slice(0, 200));
    B.x("pickerToggle('cmCity')"); const srcOpen = String(documentStub.getElementById('pk_cmCity_host').innerHTML || '');
    ok('ش١٨ · المصدر: مدن المحتوى بعدّادها (Paris 2 · Rome 1 · All cities 3) والمختارة بنقطة', srcOpen.includes('Paris</span><span><span class="cnt-num">2</span>') && srcOpen.includes('Rome</span><span><span class="cnt-num">1</span>') && srcOpen.includes('All cities') && srcOpen.includes('class="dot"'), srcOpen.slice(0, 200));
    B.x("(function(){ var s = " + prev + "; pickerRowClose('cmCity'); __cmCityRows = []; delete __cmSummary.places; delete __cmSummary.trips; communityScreen = 'source'; communityTab = s.t; communityScreenState.places.city = s.c; communityScreenState.trips.city = s.c; })()");
  ...
  await must('ش١٩ · N-065 إضافة مكان إلى رحلة بثلاث حالات على اللوحة المشتركة · الرجوع للأصل · N-076 بطاقة مكاني · N-079 بحث الاسم بالمدينة — ر٧٠و', async () => {
    ...
    ok('ش١٩ · N-080 · r72w: مكان رحلة الآخرين بالبطاقة الموحَّدة داخل مجموعة: Maps · مفكرة · إلى رحلتي · 📤 · ⚑ (بصاحبها zed)', ct.includes('Maps ↗') && ct.includes('pc-a bmk') && ct.includes("cmAddOthersPlaceToTrip('paris', '" + CAT1 + "', 'p9', 'uZED', 'Cafe Z')") && ct.includes("cmExportOthersPlace('oid:uZED:paris:p9', 'Cafe Z', 'https://maps.app.goo.gl/z', 'zed'") && ct.includes('class="pgroup"') && ct.includes('class="pc-a flag'), ct.slice(0, 200));
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    const iB = g.indexOf('>Badr<'), iA = g.indexOf('>Amal<');
    ok('١٦هـ · الشبكة: الموثَّقون فقط (لا Cara) · الأحدث تحديثًا أولًا (Amal 2d قبل Badr 9d) · دوائر بالحرفين · البيانات من قوائمه العامة (Amal: 1 city · 1 trip · Badr بلا محتوى عام) · متابعون بإذنه · updated · «You» على حسابي · صف الرأس', iB > -1 && iA > -1 && iA < iB && !g.includes('Cara') && g.includes('class="curring">AM<') && g.includes('class="curpill">') && g.includes('cities # 1 · trips # 1') && g.includes('— no public content yet') && g.includes('followers # 12') && !g.includes('followers # 3') && g.includes('updated 2d ago') && g.includes('class="curyou">You<') && g.includes('placeholder="Type a name…"') && g.includes('pick a curator'), 'iB=' + iB + ' iA=' + iA + ' metas=' + (g.match(/class="curmeta">[^<]*/g) || []).join(' | ').slice(0, 300));
    ...
    ok('١٦هـ · الترتيب: Most viewed (Amal قبل Badr) · Recently updated (Amal قبل Badr — الافتراضي) · Following شريحة · سطر السياق يسمّي الترتيب · لا Your city first ولا Followers كترتيب', gv.indexOf('>Amal<') < gv.indexOf('>Badr<') && gu.indexOf('>Amal<') < gu.indexOf('>Badr<') && gv.includes('· most viewed') && gu.includes('· recently updated') && gu.includes('>Following<') && !gu.includes('Your city first') && !gu.includes('Most followed'), '');
    ...
    ok('١٦هـ · بحث الاسم يرشّح (Amal وحدها) · مرشِّح المدينة يرشّح (Badr وحده)', g2.includes('>Amal<') && !g2.includes('>Badr<') && g3.includes('>Badr<') && !g3.includes('>Amal<'), '');
    B.x("curFilterCity = ''; curOpen('cur_a')"); await new Promise(function(r){ setTimeout(r, 80); }); const pg = screen('curatorsBody');
    ok('١٦هـ · صفحة المنتقي: رأس القالب (الحرفان · الاسم · ✧ Curator · Paris من قائمته · ＋ Follow) · النبذة · الأرقام · ✉ Contact · السطر التعريفي · الشرائح المشتقة من الاسم (Amal’s …) · Cities · 1 كبيرة بقائمة منسدلة · Search · لا مدن بالرأس · Top places · الافتراضي Cities بقائمتها مفتوحة (r72q-1)', pg.includes('class="curava">AM<') && pg.includes('Amal <span class="curvb">✧ Curator</span>') && !pg.includes('class="curmt3">') && pg.includes('＋ Follow') && pg.includes('Coffee first') && pg.includes('followers <b># 12</b>') && pg.includes('cities <b># 1</b>') && pg.includes('views <b># 40</b>') && pg.includes('✉ Contact') && pg.includes('Picks with a personal taste') && !pg.includes('🏠') && pg.includes('<b>Amal’s</b>latest updated places') && pg.includes('<b>All Amal’s</b>places lists # 1') && pg.includes('<b>All Amal’s</b>trips # 1') && pg.includes('class="curchip big on" onclick="curCitiesChip()">Cities # 1<span class="s">⌃') && pg.includes('placeholder="Search places…"') && pg.includes('Paris · 3 places · 1 trip') && pg.includes('Top places <b># 1</b></span>') === false && pg.includes('Top places <b># '), pg.slice(0, 300));
    ...
    ok('١٦هـ · بحث أماكنه: بطاقة برأس (Paris · Specialty Coffee · Open list →) والمكان بصف الآخرين الموحَّد (Maps ↗ · مفكرة · 🧳 · 📤 · Save)', sr.includes('Paris · Specialty Coffee') && sr.includes('>Ten Belles<') && sr.includes('Open list →') && sr.includes('class="pcard') && sr.includes('Maps ↗') && sr.includes('class="pc-a bmk') && sr.includes('>🧳<') && sr.includes('class="pc-a flag') && !sr.includes('Place des Vosges'), sr.slice(0, 200));
    ...
    const outsideGrid = dd.indexOf('<div class="panel bare" id="pk_curCities">') > dd.indexOf('placeholder="Search places…"'); // r72q-1: اللوحة بعد الصف الثاني // ر٧٢-أ-١ي: اللوحة بعد إغلاق الصف الأول وقبل الثاني — لا خلية بالشبكة
    ...
    ok('١٦هـ · المدينة المختارة تُعرض بالمكان نفسه تحت الشرائح (الرأس والشرائح باقيان · الشريحة تحمل اسمها Paris) · للزائر بلا رأي: لا بطاقة رأي أصلًا (r72p — قرار المالك) · مبدّل Top places · 0 / All places · 3 / Trips · 1 · لا Top places بعد', cty.includes('class="curidn"') && cty.includes('Cities # 1<span class="s">Paris ') && !cty.includes('No notes yet') && !cty.includes('Best time of year') && !cty.includes('class="curfacts"') && cty.includes('<small>Paris</small>Top places # 0') && cty.includes('<small>Paris</small>All places # 3') && cty.includes('<small>Paris</small>All trips # 1') && cty.includes('← Curators') && cty.includes('No top places in Paris yet'), cty.slice(0, 200));
    // r72p (١): رأي المدينة — يُكتب من صاحبه ويُقرأ للزائر بحقوله المكتوبة فقط
    await store.set('curatorCityNotes/cur_a__paris', { uid: 'cur_a', cityId: 'paris', text: 'Coffee first, then the river.', bestTime: 'late September', returns: 'yes', updatedAt: 1 });
    B.x("Object.keys(curNotesCache).forEach(function(k){ delete curNotesCache[k]; })"); await B.x('renderCuratorsBody()'); await new Promise(function(r){ setTimeout(r, 60); }); const ctyN = screen('curatorsBody');
    ok('١٦هـ · r72p: رأي المدينة يُعرض للزائر (النص · Best time of year · I return = Yes) ولا يعرض الحقول الفارغة (What I love)', ctyN.includes('Coffee first, then the river.') && ctyN.includes('<b>Best time of year</b> late September') && ctyN.includes('Yes — I keep coming back') && !ctyN.includes('What I love') && ctyN.includes('class="curlines"'), ctyN.slice(0, 200));
    store.delete('curatorCityNotes/cur_a__paris'); B.x("Object.keys(curNotesCache).forEach(function(k){ delete curNotesCache[k]; })");
    ...
    const iYou = g4.indexOf('class="curyou">You<'), iAm = g4.indexOf('>Amal<');
    ...
    ok('١٦هـ · «You» أول الشبكة · ⭐ Top place: رسالة لغير المنتقي وتفعيل للمنتقي · المزامنة بالمسارات الثلاثة ولا تعدّ قائمة فارغة · «Notes:» برأس بصف مكاني · لا System status بالدرج · Contact دائمًا', iYou > -1 && iYou < iAm && star === '{"a":true,"b":true}' && syncNeedle && tplCount("'<div class=\"pc-lab pc-more\">Note</div>", 1).ok && tplCount('onclick="openFromDrawer(openSystemStatusModal)"', 0).ok && tplCount('✉ Contact <span class="stage">not set</span>', 1).ok && tplCount('class="curic flag"', 1).ok, 'you=' + iYou + ' am=' + iAm + ' star=' + star);
    const rowStar = tplCount("cls: 'star' + (pl.topPlace ? ' on' : ''), html: '★', on: \"plToggleTopRow('", 1).ok && tplCount('function plToggleTopRow(catId, i)', 1).ok;
    ...
    // ر٧٢-أ-٢د: Save = نسخة بإسناد إلى رحلاتي · الشريحتان تعرضان نسخي بمصدرها · طبقة الشخص من شريحة الرحلات تعرض الرحلات فقط
    await store.set('trips/tr_pub9', { ownerId: 'cur_a', ownerName: 'Amal', cityId: 'paris', cityName: 'Paris', customLabel: 'Coffee days', type: 'city', public: true, saveCount: 0, days: [{ dayNumber: 1, places: { coffee: [{ id: 'q1', name: 'Café Mélodie', url: 'https://maps.app.goo.gl/m' }] } }] });
    ...
    // r72n (٢): Saved ✓ يُضغط ← تأكيد ← حذف نسختي · (٤) لا Save على محتواي
    await store.set('trips/tr_pub11', { ownerId: 'cur_a', cityId: 'paris', cityName: 'Paris', customLabel: 'Bakeries', type: 'city', public: true, saveCount: 0, days: [] });
    await B.x("copyOthersTrip('tr_pub11')"); const hadCopy = !!B.x("tripCopyOf('tr_pub11')");
    const cardSaved = B.x("othersCard({ title: 'X', sub: '', bmOn: false, cnt: 0, bmHandler: 'void(0)', saveHandler: \"copyOthersTrip('tr_pub11')\", savedOn: true, unsaveHandler: \"removeMyTripCopy('tr_pub11')\", exportHandler: 'void(0)', openHandler: 'void(0)' })");
    const cardSelf = B.x("othersCard({ title: 'X', sub: '', bmSelf: true, selfKind: 'trip', cnt: 0, saveHandler: 'void(0)', exportHandler: 'void(0)', openHandler: 'void(0)' })");
    B.x("openConfirmModal = function(){ return Promise.resolve(true); }"); await B.x("removeMyTripCopy('tr_pub11')"); const gone = !B.x("tripCopyOf('tr_pub11')");
    ok('١٦هـ · r72n: Saved ✓ زرٌّ يستدعي removeMyTripCopy ثم يحذف النسخة بعد التأكيد · بطاقة محتواي بلا Save', hadCopy && cardSaved.includes('onclick="removeMyTripCopy(\'tr_pub11\')"') && cardSaved.includes('>Saved ✓<') && !cardSelf.includes('>Save') && gone, JSON.stringify({ hadCopy: hadCopy, gone: gone, self: cardSelf.slice(0, 120) }));
    store.delete('trips/tr_pub11'); store.delete('copies/' + uid + '__tr_pub11');
    // r72n (٣): تبدّل المصادقة يصفّر الحالة قبل التحميل — لا أثر لحساب سابق
    const snap16 = B.x("JSON.stringify({ userTrips: userTrips, userListData: userListData, curators: curators, communityUsers: communityUsers, viewingUserUid: viewingUserUid, tripSavesMap: tripSavesMap, listBookmarksMap: listBookmarksMap, sharedCityLists: sharedCityLists, myCityListData: myCityListData, myListCityId: myListCityId, myListCountry: myListCountry, myCityListLoadedFor: myCityListLoadedFor })");
    B.x("userTrips = [{ id: 'ghost', ownerId: 'x', cityName: 'Ghost', days: [] }]; curators = [{ uid: 'g' }]; communityUsers = [{ uid: 'g' }]; viewingUserUid = 'g'; userListData.nickname = 'ghost'; resetSessionState()");
    const reset = B.x("JSON.stringify({ t: userTrips.length, c: curators, u: communityUsers.length, v: viewingUserUid, n: userListData.nickname, body: (document.getElementById('myTripsBody') || { innerHTML: '' }).innerHTML.includes('Loading') })");
    B.x("(function(){ var k = " + snap16 + "; userTrips = k.userTrips; userListData = k.userListData; curators = k.curators; communityUsers = k.communityUsers; viewingUserUid = k.viewingUserUid; tripSavesMap = k.tripSavesMap; listBookmarksMap = k.listBookmarksMap; sharedCityLists = k.sharedCityLists; myCityListData = k.myCityListData; myListCityId = k.myListCityId; myListCountry = k.myListCountry; myCityListLoadedFor = k.myCityListLoadedFor; })()"); // استعادة الحالة للمحطات التالية
    ok('١٦هـ · r72n: resetSessionState يصفّر الرحلات والمنتقين والمجتمع والاسم ويرسم «Loading…» · ويُستدعى عند تبدّل المصادقة', reset === '{"t":0,"c":null,"u":0,"v":null,"n":null,"body":true}' && tplCount("if (!user || !currentUser || user.uid !== currentUser.uid) resetSessionState();", 1).ok && tplCount("delete curData[uid]; if (!curIsMe(curPage)) try{ mpTrack.statsCurator(uid, 'page_view'); }catch(_){} /* r72p */", 1).ok, reset);
    ...
    ok('١٦هـ · r72p/q1: Top places بعدده الحقيقي (1) · زر ⚑ أحمر برأس صفحة الغير', pk.includes('Top places <b># 1</b>') && !pk.includes('picks') && pk.includes('class="curic flag" onclick="openReport(\'profile\', \'cur_a\')"'), pk.slice(0, 160));
    B.x("openInputModal = function(){ return Promise.resolve('please'); }"); await B.x("openCuratorRequest()"); const rq = store.get('curatorRequests/' + uid) || {};
    const nT3 = captured.toasts.length; await B.x("openCuratorRequest()"); const again2 = captured.toasts.slice(nT3).join('|');
    ok('١٦هـ · r72p: طلب الصيرورة يُنشأ مرة بحالة pending · الثانية ترد بالحالة', rq.uid === uid && rq.status === 'pending' && rq.text === 'please' && /pending/.test(again2), JSON.stringify(rq) + ' ' + again2);
    await B.x("openReport('place', 'cur_a:paris:coffee:p1')"); const reps = [...store.keys()].filter(function(k){ return k.indexOf('reports/') === 0; });
    const selfRow = B.x("othersPlaceRowHtml({ id: 'x', name: 'X', url: 'u' }, { uid: currentUser.uid, nickname: 'me' }, { id: 'paris', name: 'Paris' }, 'coffee', 'paris')");
    ok('١٦هـ · r72p: البلاغ يُكتب بحقوله (by · kind · docKey · reason) · لا زر ⚑ على مكاني', reps.length >= 1 && (store.get(reps[0]) || {}).by === uid && (store.get(reps[0]) || {}).kind === 'place' && (store.get(reps[0]) || {}).reason === 'please' && !selfRow.includes('openReport('), JSON.stringify(store.get(reps[0])) + ' ' + selfRow.slice(-80));
    const ownerOn2 = B.x("(function(){ var k = isOwner; isOwner = true; return k; })()");
    await B.x("openAdminRequests()"); const ar = String(documentStub.getElementById('dashBody').innerHTML || ''); await B.x("adminDecideRequest(" + JSON.stringify(uid) + ", 'accepted')"); const rq2 = store.get('curatorRequests/' + uid) || {}; const prof2 = store.get('communityProfiles/' + uid) || {};
    await B.x("openAdminReports()"); const arep = String(documentStub.getElementById('dashBody').innerHTML || ''); await B.x("adminDismissReport(" + JSON.stringify(reps[0].split('/')[1]) + ")"); const gone2 = !store.get(reps[0]);
    B.x("isOwner = " + ownerOn2);
    ok('١٦هـ · r72p: الإدارة — الطلبات (Accept يضع verified) · البلاغات (Open · Dismiss يحذف)', ar.includes('>Accept<') && rq2.status === 'accepted' && prof2.verified === true && arep.includes('Reason:') && gone2, JSON.stringify({ st: rq2.status, v: prof2.verified, gone: gone2 }));
    const cs = B.x("(function(){ try{ localStorage.clear(); }catch(_){} var n0 = 0; mpTrack.statsCurator('cur_a00000000000000000', 'page_view'); mpTrack.statsCurator('cur_a00000000000000000', 'page_view'); mpTrack.statsCurator('cur_a00000000000000000', 'contact_click'); return 'ok'; })()");
    ok('١٦هـ · r72p: إحصاء المنتقي — الدالة موجودة بمفاتيحها الستة (page_view مرة يوميًّا · follow · contact_click · ref_social · ref_card) · trip_built بأول حفظ · Resend يعمل · رمز فشل سجل النسخ', cs === 'ok' && tplCount("add('stats_curators/' + uid + '__' + day, field, 1);", 1).ok && tplCount("if (localStorage.getItem(k)) return; localStorage.setItem(k, '1');", 1).ok && tplCount("mpTrack.statsCity(trip.cityId, 'trip_built')", 1).ok && tplCount('onclick="resendVerification()"', 1).ok && tplCount("showToast('Could not record · '", 1).ok, '');
    store.delete('curatorRequests/' + uid); await store.set('communityProfiles/' + uid, { nickname: 'Me', verified: true, publicCityIds: [], updatedAt: Date.now(), hasAnyPublicContent: false });
    // r72q-1: الشرائح تتغير بتغيّر الاسم والمدينة (اشتقاق لحظة الرسم) · الشبكة كبسولات · نافذة الرأي بقشرة التطبيق وتلميحاتها
    await store.set('communityProfiles/cur_z', { nickname: 'Louis', verified: true, publicCityIds: ['paris'], hasAnyPublicContent: true, updatedAt: 5 });
    await store.set('userCityLists/cur_z_paris', { ownerId: 'cur_z', cityId: 'paris', cityName: 'Paris', public: true, categories: { coffee: { places: [{ id: 'z1', name: 'Z', url: 'u' }] } } });
    B.x("curators = (curators || []).concat([{ uid: 'cur_z', nickname: 'Louis', verified: true, publicCityIds: ['paris'], hasAnyPublicContent: true }]); curOpen('cur_z')"); await new Promise(function(r){ setTimeout(r, 80); }); const pz = screen('curatorsBody');
    B.x("curPickCityFromChip('paris')"); await new Promise(function(r){ setTimeout(r, 60); }); const pz2 = screen('curatorsBody');
    ok('١٦هـ · r72q-1: الاشتقاق — الاسم المنتهي بـs («Louis’») بالشرائح · اسم المدينة بشرائح شاشتها (Paris …) · الشبكة كبسولة بالحرفين والاسم', pz.includes('<b>Louis’</b>latest updated places') && pz.includes('<b>All Louis’</b>trips') && pz2.includes('<small>Paris</small>Top places # 0') && pz2.includes('<small>Paris</small>All places # 1') && tplCount('<span class="curpill"><span class="curring">', 1).ok, pz.slice(0, 160));
    ...
    B.x("communityTab = 'places'; cmSortOpen = true; renderCommunityModal()"); await new Promise(function(r){ setTimeout(r, 80); }); const mk = screen('communityBody');
    const chk = { noGlobe: !mk.includes('🌍'), noBack: !mk.includes('backchip'), switchFirst: mk.indexOf('>Places<') < mk.indexOf('All users places lists'), all: mk.includes('All users places lists <span class="srcsub">'), search: mk.includes('>Search by username<'), sort: /All users places lists <span class="srcsub">· /.test(mk), panel: mk.includes('All · recent') && mk.includes('Places most saved by users'), mine: mk.includes('My bookmarked from users'), noShared: !mk.includes('Shared with me') };
    ok('١٦هـ · r72s: الشاشة الواحدة — بلا 🌍 ولا عودة · المبدّل أولًا · All users + Search by username · My bookmarked + Sort by (بخيار All · recent والثلاثة) · لا Shared with me', Object.values(chk).every(Boolean) && tplCount("places: { city: '', marked: false, sort: 'recent' }", 1).ok, JSON.stringify(chk));
    ...
    // r72s: My bookmarked from users — تجميع بالمستخدم: قوائمه المميَّزة بطاقاتٍ وأماكنه المميَّزة بمجموعة رأسها اسم القائمة · طبقة الشخص بالاتحاد · لا وميض قبل البوابة
    await store.set('userCityLists/cur_a_rome', { ownerId: 'cur_a', cityId: 'rome', cityName: 'Rome', public: true, viewCount: 4, bookmarkCount: 1, categories: { pizza: { active: true, places: [{ id: 'r1', name: 'Bonci', url: 'https://maps.app.goo.gl/b' }] } } });
    B.x("listBookmarksMap = listBookmarksMap || {}; listBookmarksMap['cur_a_rome'] = { ownerUid: 'cur_a', cityId: 'rome', at: 1 }; userListData.placeBookmarks = userListData.placeBookmarks || {}; var m = userListData.placeBookmarks; m['oid:cur_a:paris:p9'] = { url: 'https://maps.app.goo.gl/p9', key: 'oid:cur_a:paris:p9', name: 'Ten Belles', category: 'coffee', cityId: 'paris', area: '10th', ownerUid: 'cur_a' }; communityTab = 'places'; communityScreenState.places.city = ''");
    ...
    // r72u: قراءة قائمة الآخرين المقسَّمة تقيّد الاستعلام بـpublic (قاعدة Firestore) · الخاصة غير المشارَكة تعود بلا أماكن · المشارَكة معي بقيد array-contains
    await store.set('userCityLists/cur_a_splitpub', { ownerId: 'cur_a', cityId: 'splitpub', cityName: 'SP', public: true, sharedWith: [], index: [{ id: 'x1', name: 'X1', catId: 'coffee' }], splitV: 1 });
    await store.set('userCityListCats/cur_a_splitpub_coffee', { ownerId: 'cur_a', cityId: 'splitpub', catId: 'coffee', public: true, sharedWith: [], places: [{ id: 'x1', name: 'X1', url: 'u' }] });
    await store.set('userCityLists/cur_a_splitpriv', { ownerId: 'cur_a', cityId: 'splitpriv', public: false, sharedWith: [], index: [], splitV: 1 });
    const other = await B.x("mpData.cityLists.get('cur_a', 'splitpub')"); const priv = await B.x("mpData.cityLists.get('cur_a', 'splitpriv')");
    ok('١٦هـ · r72u: قائمة الآخرين العامة المقسَّمة تُقرأ كاملة (X1) · الخاصة غير المشارَكة تعود بلا أماكن (بلا استعلام يخالف القاعدة)', other && other.categories && other.categories.coffee && other.categories.coffee.places[0].name === 'X1' && priv && priv.categories && Object.keys(priv.categories).length === 0 && tplCount("if (parent.public === true) q = q.where('public', '==', true); else if (me && (parent.sharedWith || []).indexOf(me) >= 0) q = q.where('sharedWith', 'array-contains', me);", 1).ok, JSON.stringify({ o: !!other, p: priv && Object.keys(priv.categories || {}) }));
    store.delete('userCityLists/cur_a_splitpub'); store.delete('userCityListCats/cur_a_splitpub_coffee'); store.delete('userCityLists/cur_a_splitpriv');
    // r72v: الصف الأول شريحة واحدة كامل الصف بعدده وسهمه (الأماكن والرحلات) · بطاقة رحلتي بالقشرة الموحَّدة داخل مجموعة المدينة
    B.x("placesSource = 'mine'; plShowOpen = false; plSetSrcTitle(7)"); const t1 = String((documentStub.getElementById('plSrcTitle') || { innerHTML: '' }).innerHTML || ''); B.x("plToggleShowPanel()"); const t2 = String((documentStub.getElementById('plSrcTitle') || { innerHTML: '' }).innerHTML || ''); const pnl = String((documentStub.getElementById('plShowPanel') || { innerHTML: '' }).innerHTML || ''); B.x("plToggleShowPanel()");
    ok('١٦هـ · r72v: شريحة الأماكن الواحدة «My places # 7 ⌄» تفتح قائمة المصادر وتصير ⌃ · بلا Show مستقلة · الرحلات كذلك', t1.includes('<b>My places</b> <span class="dim"># 7</span> ⌄') && t2.includes('⌃') && pnl.includes('Shared with me') && tplCount('id="plShowBtn"', 0).ok && tplCount('id="tripShowBtn"', 0).ok && tplCount('class="chipgrid c1full" id="tripSrcRow"', 1).ok, t1 + ' | ' + t2.slice(0, 60));
    const tripCard = B.x("(function(){ var k = { s: tripsSource, u: userTrips }; tripsSource = 'mine'; userTrips = [{ id: 'tv1', ownerId: currentUser.uid, type: 'city', cityId: 'paris', cityName: 'Paris', customLabel: 'Bakeries', public: true, sharedWith: [], days: [{ dayNumber: 1, places: {} }] }]; tripSourceFilter = null; currentTripId = null; renderMyTripsModal(); var h = document.getElementById('myTripsBody').innerHTML; tripsSource = k.s; userTrips = k.u; return h; })()");
    ok('١٦هـ · r72v: بطاقة رحلتي: مجموعة عاجية برأس المدينة (Paris # 1) · بطاقة كريمية بالاسم وType/Places/Days · Actions: Open → · 🔖 · 📤 · 🌐 · مشاركة · ✏️', tripCard.includes('class="pg-title">Paris <span class="pg-n"># 1</span>') && tripCard.includes('class="pcard open"') && tripCard.includes('>Places</div><div class="pc-val pc-area"># 0<') && tripCard.includes('>Days</div><div class="pc-val pc-area"># 1<') && tripCard.includes('>Open →<') && tripCard.includes('🌐') && tripCard.includes('>✏️<'), tripCard.slice(0, 200));
    // r72w: الملاحظات الخمس — تفصيل القائمة بالمجموعة بلا أيقونات · رحلة الآخرين بالبطاقة · نص الترتيب بلا خفوت · الأعداد بالقوائم المنسدلة · عدّاد النسخ ورمز فشله
    B.x("plCatsAll().slice(0, 3)"); const noIcon = B.x("plCatsAll().every(function(c){ return !c.icon; })");
    B.x("cmLastRows = [{ bookmarkCount: 2, viewCount: 3, copyCount: 0 }, { bookmarkCount: 0, viewCount: 0, copyCount: 1 }]; communityTab = 'places'"); const cnts = B.x("JSON.stringify(cmSortCounts())");
    ...
    ok('١٦هـ · ز-١-ج-٢: لا ترقيع مدن (لا geoCityIdOf ولا KNOWN_CITY_NAMES ولا رسالة اختلاف المدينة ولا مدن مواسم ولا إخفاء) · CARTO · v3.11 · اللصق المباشر · «أنا هنا» · سجل الهوية قبل المطابقة وبعد التأكيد (بحارس الكيلومتر)', tplCount('geoCityIdOf', 0).ok && tplCount('KNOWN_CITY_NAMES', 0).ok && tplCount('geoSwitchCity', 0).ok && tplCount('cityOpenNow', 0).ok && tplCount('hiddenCities', 0).ok && tplCount('basemaps.cartocdn.com', 3).ok && tplCount('tile.openstreetmap.org', 0).ok && tplCount("const EXPECTED_RULES = 'v3.11';", 1).ok && tplCount('onpaste="plOnPaste(event)"', 1).ok && tplCount('function geoHere(){', 1).ok && tplCount('await mpData.identity.get(idKey)', 1).ok && tplCount('geoDistM(prev, plGeoResult.geo) > 1000', 1).ok && tplCount("No place suggestions for this city yet — drop the pin.", 1).ok, '');
    const gc = B.x("(function(){ var k = userListData.customCities; userListData.customCities = [{ id: '290340', name: 'Manama', country: 'Bahrain', lat: 26.2285, lng: 50.586 }]; var a = JSON.stringify(cityCenterOf('290340')); userListData.customCities = k; return a; })()");
    ok('١٦هـ · ز-١-ج-٢: مركز المدينة من «مدني» بمعرّف المعجم (المنامة 290340 → 26.2285,50.586)', gc === '[26.2285,50.586]', gc);
    const arr = B.x("(function(){ var k = myListCityId; myListCityId = 'paris'; var a = parisArrFrom('8 Rue Tiquetonne, 75002 Paris'), b = parisArrFrom('12 Rue de Rivoli, 75001 Paris'), c = parisArrFrom('Al Imam Saud Rd, Riyadh 13315'); myListCityId = 'riyadh'; var d = parisArrFrom('8 Rue Tiquetonne, 75002 Paris'); myListCityId = k; return [a, b, c, d].join('|'); })()");
    ok('١٦هـ · ز-١-ب: باريس — الحي بالرقم من الرمز البريدي (75002 → 2e arr. · 75001 → 1er arr.) ولا يسري خارج باريس', arr === '2e arr.|1er arr.||', arr);
    ...
    // ز-١-ج-٢: سجل الهوية — موضع مصدَّق سابقًا يُقترح أولًا · التأكيد يكتبه · حارس الكيلومتر
    const idKey = B.x("mpData.identity.keyOf(myListCityId, 'Known Spot')"); await store.set('placeIdentity/' + idKey, { lat: 48.87, lng: 2.35, source: 'user_pin', n: 'Known Spot', cityId: 'paris', by: 'uX', at: 1 });
    B.x("window.__mpGeoStub = { candidates: [] }; plEdit = null; plModalCat = '" + CAT1 + "'; document.getElementById('plName').value = 'Known Spot'; document.getElementById('plUrl').value = 'https://maps.app.goo.gl/k1'; plGeoConfirmed = false; plGeoResult = null");
    await B.x('savePlPlace()'); await new Promise(r => setTimeout(r, 40)); const kst = B.x("document.getElementById('geoStatus').textContent"); const kc = B.x("document.getElementById('geoCands').innerHTML");
    await B.x('geoConfirm()'); await new Promise(r => setTimeout(r, 60)); const idRec = store.get('placeIdentity/' + idKey) || {};
    ok('١٦هـ · ز-١-ج-٢: سجل الهوية — «Pinned before by MyPickz users» والمرشَّح «confirmed» أولًا · التأكيد يعيد كتابة السجل بصاحبه (by)', /Pinned before by MyPickz users/.test(kst) && kc.includes('Known Spot · confirmed') && idRec.by === B.x('currentUser.uid') && Math.abs(idRec.lat - 48.87) < 0.001, kst + ' | ' + JSON.stringify(idRec));
    B.x("window.__mpGeoStub = { noData: true, candidates: [] }; plEdit = null; plModalCat = '" + CAT1 + "'; document.getElementById('plName').value = 'Nowhere Cafe'; document.getElementById('plUrl').value = 'https://maps.app.goo.gl/k2'; plGeoConfirmed = false; plGeoResult = null");
    await B.x('savePlPlace()'); await new Promise(r => setTimeout(r, 40)); const nst = B.x("document.getElementById('geoStatus').textContent"); await B.x('geoSkip()'); await new Promise(r => setTimeout(r, 40));
    ok('١٦هـ · ز-١-ج-٢: مدينة بلا بيانات → «No place suggestions for this city yet — drop the pin.» والحفظ يبقى ممكنًا', /No place suggestions for this city yet/.test(nst), nst);
    ...
    const cOthers = B.x("othersPlaceRowHtml(" + JSON.stringify(pl6) + ", { uid: 'cur_a', nickname: 'Amal' }, { id: 'paris', name: 'Paris' }, 'coffee', 'paris')");
    const cSelf = B.x("othersPlaceRowHtml(" + JSON.stringify(pl6) + ", { uid: currentUser.uid, nickname: 'me' }, { id: 'paris', name: 'Paris' }, 'coffee', 'paris')");
    ...
    ok('١٦هـ · r72q-2: لا أيقونة برؤوس الأقسام والعدد بـ# · شريحة المدينة بسطرين · Cities # n · العدّاد بالشبكة #', tplCount("html += placeGroupHtml(cat.name, visible.length, __catRight, __cards);", 1).ok && tplCount("'<div class=\"pl-cathead\"><span>' + cat.icon", 0).ok && tplCount("<small>' + cn + '</small>Top places # '", 1).ok && tplCount("Cities # ' + d.cities.length", 1).ok && tplCount("'cities # ' + nC + ' · trips # ' + nT", 1).ok && tplCount('<span class="trip-cat">${meta.label}</span>', 0).ok && tplCount('html += placeGroupHtml(meta.label, places.length, __right, __tcards);', 1).ok && tplCount('html += placeGroupHtml(meta.label, day.places[catId].length, \'\', __tcards);', 1).ok, ''); // r72r-1 · r72w: رحلة الآخرين مجموعة أيضًا
    cap('curators.follow');
  ...
  await must('ش١ · ٦/أ الأماكن: صف الأفعال الخمسة بلا سلة (الشاشة)', async () => {
    ...
    const a = sees('plBody', ['Maps', 'pc-a bmk', '📤', '✏️']); const b = notSees('plBody', ['🗑', 'plDeleteRow(']); // r72q-2
  ...
  await must('ش٥ · ٦/د رأس السوق: مبدل نقي ثم شرائح التصفح والفرز (الشاشة)', async () => {
    B.x("communityTab = 'places'; renderCommunityModal()"); await new Promise(r => setTimeout(r, 30));
    const root = inOrder('communityBody', ['chipgrid c2', '>Places<', '>Trips<', 'id="cmCityRow"', 'All users places lists <span class="srcsub">· ', 'Search by username', 'chipgrid c1', 'My bookmarked from users']); // r72u · r72w
    const rootNo = notSees('communityBody', ['backchip', '🌍', 'Shared with me', 'Browse Places', 'Sort by:']);
  ...
  await must('ش١٢ · القرار ١١ (٦): العودة للجذر ثم فتح المصدر يعيد حالته (المدينة والشريحة)', async () => {
    ...
    B.x("cmPickTab('trips')"); B.x("cmPickTab('places')"); await new Promise(r => setTimeout(r, 30)); // r72s: التبديل بالمبدّل يحفظ حالة كل مصدر
  ...
  await must('ش١٣ · القرار ١١ (٧): شرائح الرأس شبكة متساوية بلا تمرير — الرباعي ٢×٢', async () => {
    const a = tpl(['.chipgrid{display:grid;', '.chipgrid.c2{grid-template-columns:repeat(2', '.chipgrid.c3{grid-template-columns:repeat(3', 'class="chipgrid c1full" id="tripSrcRow"', 'class="chipgrid c3" id="tripTypeRow"', 'class="chipgrid c1full" id="plSrcRow"']);
  ...
  await must('ش١٤ · القرار ١١ (٣·٥) + N-024: الرحلات بمحدد مدينة من رحلاتك · Create trip بصف الأفعال · بطاقة الرحلة بأفعالها الستة (عرض · عام/خاص · مشاركة · مفكرة · تصدير · تحرير) و➕ داخل التحرير', async () => {
    const a = tpl(['id="tripCityPick" onclick="tripToggleCityPanel()"', 'onclick="openCreateTripFlow()">＋ Create trip', 'toggleTripPublicFor(', 'shareTripFor(', "openTripDetail('\" + t.id + \"', 'edit')"]); // r72v: البطاقة الموحَّدة
  ...
  await must('ش٨ · القوالب الساكنة: شرائح الرحلات بالحرف وOne day trip موسومة', async () => {
    const a = tpl(["['saved', 'My bookmarked'], ['savedCur', 'Saved from Curators'], ['savedCom', 'Saved from Community'], ['shared', 'Shared with me']", 'id="tripSrcTitle" onclick="tripToggleShowPanel()"']); // r72r-1: Show ⌄
  ...
  await must('ش٩ · ٦/هـ٢ صفحة المنتقي: رأس واحد وأفعال التسوية بلا تعتيم', async () => {
    const a = tplCount('← Curators', 1); const b = tpl(["const CUR_BADGE = 'Curator';", "html: 'Save <span class=\"dim\">3</span>'"]); const bNo = tplCount('>🔖 Bookmarked</span>', 0); const bOld = tplCount('verified curator', 0); // r72s: Save 3 بالبطاقة الموحَّدة · ر٧٢-أ-٢: شريحتا الحفظ بالرحلات حيّتان (لا وسم stage 3 عليهما)
  ...
  await must('ش١٠ · ٦/أ شرائح الأماكن: المنشآن معطَّلان بوسم موعدهما', async () => {
    const a = tpl(["if (id === 'curators' || id === 'community'){ showSoon('Your copies of places arrive with Save (stage 3)'); return; }", 'id="plSrcTitle" onclick="plToggleShowPanel()"']); // r72r-1
  ...
  await must('ت٠ · محرك الضغط يصل للمعالج (الضغط يبدّل مصدر المجتمع فعلًا)', async () => {
    B.x("communityTab = 'places'; renderCommunityModal()"); await new Promise(r => setTimeout(r, 30));
    const c = await click('communityBody', 'Trips'); await new Promise(r => setTimeout(r, 30)); // r72s: المبدّل يفتح الرحلات بالشاشة نفسها
    const a = sees('communityBody', ['All users trips', 'My bookmarked from users']);
    ok('ت٠ · ضغط «Trips» بمبدّل المصدر يفتح الرحلات بالشاشة الواحدة (r72s)', c.ok && a.ok, c.why + a.why);
  }, ['click.engine.reachesHandler']);

  await must('ت١ · تصفح ← تمييز بالضغط ← عودة ← إعادة فتح ← إعادة تحميل: التمييز باقٍ', async () => {
    B.x("communityTab = 'places'; renderCommunityModal()"); await new Promise(r => setTimeout(r, 40)); // r72s
    ...
    B.x("cmPickTab('trips')"); B.x("cmPickTab('places')"); await new Promise(r => setTimeout(r, 40)); // r72s
