...
(async () => {
  ...
  await must('ش١٨ · ٦/د المجتمع: الجذر والمصدر على الصيغة الموحَّدة (شريحة + حقل) بعدّاد المحتوى لكل مدينة · Go فعلًا رئيسًا — ر٧٠هـ', async () => {
    ...
    ok('ش١٨ · r72s: الشاشة الواحدة — لا مدخل ولا زر عودة · مبدّل المصدر أولًا · شريحة المدينة بصف واحد · All users + Search by username · My bookmarked + Sort by', B.x('communityScreen') === 'source' && !one.includes('backchip') && !one.includes('cmRootCity') && one.indexOf('>Places<') < one.indexOf('id="cmCityRow"') && one.includes('class="headrow pkrow"') && one.includes('>Search by username<') && one.includes('All users places lists <span class="dim">· '), one.slice(0, 200));
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    const chk = { noGlobe: !mk.includes('🌍'), noBack: !mk.includes('backchip'), switchFirst: mk.indexOf('>Places<') < mk.indexOf('All users places lists'), all: mk.includes('All users places lists <span class="dim">'), search: mk.includes('>Search by username<'), sort: /All users places lists <span class="dim">· /.test(mk), panel: mk.includes('All · recent') && mk.includes('Places most saved by users'), mine: mk.includes('My bookmarked from users'), noShared: !mk.includes('Shared with me') };
  ...
  await must('ش٥ · ٦/د رأس السوق: مبدل نقي ثم شرائح التصفح والفرز (الشاشة)', async () => {
    ...
    const root = inOrder('communityBody', ['chipgrid c2', '>Places<', '>Trips<', 'id="cmCityRow"', 'All users places lists <span class="dim">· ', 'Search by username', 'chipgrid c1', 'My bookmarked from users']); // r72u
