...
(async () => {
  ...
  await must('ش٢٢ · أ-١٢-١ (r70r): الوجهة الأماكن دائمًا والاختيار اليدوي فوقها · وسم «مدينتي» يدوي حتى مدينتين ولا يؤثر بالفتح · العودة على آخر مدينة محددة · مثال الملاحظة', async () => {
    ...
    const m1 = B.x("JSON.stringify(myCities())"); await B.x("plToggleMyCity(" + JSON.stringify(c2) + ")"); const m3 = B.x("JSON.stringify(myCities())");
    const saved = store.get('userLists/' + B.x('currentUser.uid')) || {};
    const prevCountry = B.x('myListCountry'); B.x("myListCountry = 'Testland'; plPanel = 'cities'; __pk.plCities = __pk.plCities || {}; __pk.plCities.edit = false; renderPlacesMine()"); const pane = screen('plBody'); B.x("plPanel = null; myListCountry = " + JSON.stringify(prevCountry));
    ...
    ok('ش٢٢ · r70p2: وسم مدينتي يُقرأ عند التحميل فلا يضيع بالخروج والدخول · مثال الملاحظة بالنافذة', reload === '["mylist_alpha"]' && tplCount('placeholder="e.g. best pancakes before 11 — book ahead"', 1).ok, 'got ' + reload);
    ...
    B.x("(function(){ var s = " + prev + "; userListData.defaultTab = s.dt; userListData.myCities = s.mc; userListData.cityPlaceCounts = s.cpc; userListData.listCity = s.lc; })()");
    if (typeof prevCC !== 'undefined') B.x("userListData.customCities = " + prevCC);
    await B.x("mpData.userLists.merge(currentUser.uid, { defaultTab: userListData.defaultTab, myCities: userListData.myCities })");
    }
  }, []);
  ...
  await must('١٦هـ · ر٧٢-أ-١ (إعادة البناء من المسودة v2): شبكة المنتقين بالبيانات · صفحة المنتقي بالرأس والشرائح الست · شاشة المدينة · حالة «أنا» · طبقة الشخص بشريطها · الوسم بيد المالك', async () => {
    ...
    const prevTab16 = B.x('currentTab'); B.x("currentTab = 'Curators'; curators = null; curPage = null; curCity = null; curChip = null; curFilterCity = ''; curOnly = 'all'; curNameQ = ''; curSearchQ = ''; curCityOpen = false; userListData.myCities = ['mylist_alpha']; Object.keys(curData).forEach(function(k){ delete curData[k]; })");
    ...
    ok('١٦هـ · الشبكة: الموثَّقون فقط (لا Cara) · من علّم مدينتي أولًا (Badr قبل Amal) · دوائر بالحرفين · البيانات من قوائمه العامة (Amal: 1 city · 1 trip · Badr بلا محتوى عام) · متابعون بإذنه · updated · «You» على حسابي · صف الرأس', iB > -1 && iA > -1 && iB < iA && !g.includes('Cara') && g.includes('class="curring">AK<') && g.includes('1 city · 1 trip') && g.includes('— no public content yet') && g.includes('12 followers') && !g.includes('3 followers') && g.includes('updated 2d ago') && g.includes('class="curyou">You<') && g.includes('placeholder="Type a name…"') && g.includes('pick a curator'), 'iB=' + iB + ' iA=' + iA + ' metas=' + (g.match(/class="curmeta">[^<]*/g) || []).join(' | ').slice(0, 300));
    ...
    B.x("curSort = 'views'"); await B.x('renderCuratorsBody()'); const gv = screen('curatorsBody'); B.x("curSort = 'updated'"); await B.x('renderCuratorsBody()'); const gu = screen('curatorsBody'); B.x("curSort = 'city'"); await B.x('renderCuratorsBody()'); const gc = screen('curatorsBody');
    ok('١٦هـ · الترتيب: Most viewed (Amal قبل Badr) · Recently updated (Amal قبل Badr) · Your city first (Badr قبل Amal) · سطر السياق يسمّي الترتيب · لا Followers كترتيب', gv.indexOf('>Amal K.<') < gv.indexOf('>Badr<') && gu.indexOf('>Amal K.<') < gu.indexOf('>Badr<') && gc.indexOf('>Badr<') < gc.indexOf('>Amal K.<') && gv.includes('· most viewed') && gc.includes('· your city first') && gc.includes('>Following<') && !gc.includes('Most followed'), '');
    ...
    ok('١٦هـ · إعادة ضغط Cities تطوي القائمة (choose ⌄)', fold.includes('Cities · 1<span class="s">choose ⌄') && !fold.includes('id="pk_curCities'), fold.slice(0, 120));
    ...
    ok('١٦هـ · شاشة المدينة: هيكل الرأي (٥ حقول) · مبدّل Top places · 0 / All places · 3 / Trips · 1 · العودة ← Amal K. · لا Top places بعد', cty.includes('Best time of year') && cty.includes('I return') && cty.includes('Top places · 0') && cty.includes('All places · 3') && cty.includes('Trips · 1') && cty.includes('← Amal K.') && cty.includes('No top places in Paris yet'), cty.slice(0, 200));
    ...
    const rowStar = tplCount('class="topbtn\' + (pl.topPlace ? \' on\' : \'\') + \'" onclick="plToggleTopRow(', 1).ok && tplCount('function plToggleTopRow(catId, i)', 1).ok;
    const personCard = tplCount("saveMsg: 'Copy this list into yours — stage 3', exportHandler: \"exportOtherList('\" + attrStr(u.uid) + \"', '\" + attrStr(c.id) + \"')\", openHandler: \"openCommunityCityList('\" + attrStr(c.id) + \"')\"", 1).ok;
    ...
    B.x("curPage = null; curCity = null; curators = null; curArchiveOf = null; viewingUserUid = null; viewingUserData = null; communityScreen = 'root'; communityTab = 'places'; userListData.myCities = []; Object.keys(curData).forEach(function(k){ delete curData[k]; }); currentTab = " + JSON.stringify(prevTab16));
