...
(async () => {
  ...
  for (const doc of snap.docs){
    ...
    const cityId = String(g.id); const cats = {}; let skippedPrivate = 0; Object.keys(links).forEach(k => { const e = links[k]; if (!e || !Array.isArray(e.places)) return; let id = CAT_ID_MAP[k] || k; const ps = e.places.filter(p => p && (p.name || p.url)).map(p => Object.assign({}, p, { id: p.id || ('d_' + Math.random().toString(36).slice(2, 10)) })); if (!ps.length) return; if (PRIVATE.includes(id)){ ps.forEach(p => console.log('  SKIPPED private category', k, '·', p.name || p.url)); skippedPrivate += ps.length; return; } (cats[id] = cats[id] || { active: true, places: [] }).places.push(...ps); });
    totalBefore -= skippedPrivate; // الأماكن بالتصنيفات الخاصة لا تدخل قائمة عامة (القواعد) — تُطبع لقرار المالك
